...
const RentedUnit = (props) => {
    ...
    const ratingValue = rental.unit.rentals
        .map((rental) => {
            let out = rental.reviews.find((review) => {
                return review.user_id === authCtx.user.id && review.star
            })
            return out ? out.star : null // Return the star value or null if no matching review
        })
        .find((value) => value !== null) // Get the first non-null value

    console.log(
        'rentalsss: ',
        rental.unit.rentals.map((rental) => {
            console.log(rental)
        })
    )
